...
const getCountryInfo = async (country) => {
  const res = await runCompletion(
    `Tell me ${country}'s coountry name, country code, summary, official language, area, population, GDP, area ranking, population ranking, GDP ranking.
     Your response should be in JSON format with 10 parameters 'name', 'summary', 'countryCode', 'officialLangage', 'area', 'population', 'GDP', 'areaRanking', 'poplationRanking', 'GDPRanking'
     Don't include commas in numeric values.
     Use the most recent data available.
     Don't omit the coountry name.
     Return the country code in two uppercase digits.
     Summary should include a summary of the country, approximately 500 words.
     Summary should not include data for a specific year so that it does not need to be updated.
     ...
     All rankings return how many places in the country.
     `
  );
  // {
  //   "name": "United Kingdom"
