...
const getCountryInfo = async (country) => {
  const res = await runCompletion(
    `Please provide the following information for ${country}: name, summary, countryCode, officialLanguage, area, population, GDP, areaRanking, populationRanking, and GDPRanking.
     The response should be in JSON format and based on the most recent data available.
     Ensure that you include the country's name, use two uppercase digits for the country code, and provide a summary of approximately 50 words that does not include data for a specific year.
     If there is more than one official language, please return an array of official languages.
     Do not include commas in the area, population, or GDP figures, and use km2 as the unit of area.
     Please provide only numbers for the areaRanking, populationRanking, and GDPRanking.
     The data for population, area, GDP, populationRanking, areaRanking, and GDPRanking should be same as that found on Wikipedia.
     `
  );
  console.log(res);
  // {
  //   "name": "United Kingdom"
  //   "code": "UB"
  //   "summary": "unitd kingdom is a country in europe"
